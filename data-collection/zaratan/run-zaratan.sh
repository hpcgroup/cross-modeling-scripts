...
source /scratch/zt1/project/bhatele-lab/user/amovsesy/performance-modeling/cross-modeling-scripts/data-collection/utilities.bash
...
START=$(timestamp)
echo "srun -n ${NRANKS} hpcrun -o \"hpctoolkit-measurements\" ${HPCRUN_EVENTS} ${EXEC} ${ARGS}"
srun -n ${NRANKS} hpcrun -o "hpctoolkit-measurements" ${HPCRUN_EVENTS} ${EXEC} ${ARGS}
END=$(timestamp)
